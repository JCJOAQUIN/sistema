		...
		<div class="bg bg-orange-400 flex">
			<div class="w-full md:pl-20 pl-14">
				@component("components.labels.label")
					@slot('classEx')
						text-white text-center text-lg font-bold w-full align-middle py-2 count_unsafe_practice
					@endslot
					Práctica Insegura: # {{ $count_unsafe_practice }}
				@endcomponent
				@component("components.inputs.input-text")
					@slot('attributeEx')
						type="hidden" 
						name="up_id[]" 
						value="{{ isset($up) ? $up->id : 'x' }}"
					@endslot
				@endcomponent
			</div>
		...
		@component("components.containers.container-form")
					...
					foreach (App\AuditCategory::orderBy('id','asc')->get() as $category)
					{
						...
						[
							[
								...
								"selected" 		=> (isset($up) && $up->category_id == $category->id ? "selected" : "")
							]
						]);
					}
				@endphp
			...
			<div class="col-span-2">
				...
				@php
					...
					if(isset($up))
					{
						foreach (App\AuditSubcategory::where('audit_category_id',$up->category_id)->orderBy('id','asc')->get() as $subCategory)
						{
							...
							[
								[
									...
									"selected" 		=> ($up->subcategory_id == $subCategory->id ? "selected" : "")
								]
							]);
						}
					}
					...
					foreach(["1/3", "1" ,"3"] as $item)
					{
						...
						[
							[
								"value" => $item,
								"description" => $item,
								"selected" => (isset($up) && $up->dangerousness == $item ? "selected" : "")
							]
						]);
					}
				@endphp
			...
			<div class="col-span-2">
				...
				@component("components.inputs.input-text")
					@slot('attributeEx')
						...
						readonly="readonly" 
						value="{{ isset($up) ?Carbon\Carbon::createFromFormat('Y-m-d',$up->fv)->format('d-m-Y') : '' }}"
					@endslot
					@slot('classEx')
					...
					foreach(["Abierto", "Cerrado"] as $item)
					{
						...
						[
							[
								"value" =>  $value,
								"description" => $item,
								"selected" => (isset($up) && $up->status == $value ? "selected" : "")
							]
						]);
						$value++;
					}
			...
			<div class="up_before_documents hidden col-span-2 md:col-span-4 grid grid-cols-1 md:grid-cols-2 gap-6 p-2"></div>
			<div class="md:col-span-4 col-span-2 grid justify-items-center md:justify-items-start">
				@component("components.buttons.button", ["variant" => "warning"]) 
					@slot("attributeEx")
						type="button"
						name="add_doc_up_before"
						data-num-up="{{ $count_unsafe_practice }}"
					@endslot
			...
			<div class="col-span-2 md:col-span-4">
				@if(isset($up))
					@php
						...
						$modelHead = ["Nombre", "Documento"];
						$count = 1;
						foreach($up->afterDocuments as $doc)
						{
							$row =
							[
								"classEx" => "tr",
			...
			<div class="up_after_documents hidden col-span-2 md:col-span-4 grid grid-cols-1 md:grid-cols-2 gap-6 p-2"></div>
			<div class="md:col-span-4 col-span-2 grid justify-items-center md:justify-items-start">
				@component("components.buttons.button", ["variant" => "warning"]) 
					@slot("attributeEx")
						type="button"
						data-num-up="{{ $count_unsafe_practice }}" 
						name="add_doc_up_after"
					@endslot
					<span class="icon-plus"></span>
					<span>Nueva imagen</span>
				@endcomponent
