...
@section('data')
	...
	@component("components.forms.form",["attributeEx" => "id=\"formsearch\" action=\"".route('report.concentrated.excel')."\"","variant"=>"deafult"])
		@component("components.containers.container-form")
			...
			<div class="col-span-2">
				...
				@php
					...
					{
						foreach( App\Account::whereIn('idEnterprise',$enterprise)->where('account',$father)->orderBy('account','ASC')->get() as $a)
						{
	...
	$(document).ready(function()	
	{
		...
		if(type != null)
		{
			...
			if(type == 1)
			{
				var options = 
				{
					...
					title: 
					{
						text		: '{{mb_convert_case($projectname, MB_CASE_TITLE, "UTF-8")}}',
						align		: 'left',
						margin		: 10,
						offsetX		: 0,
						offsetY		: 0,
		...
		$(document).on('click','#showChartBar',function()
		{
			...
			if (idEnterprise == "" || father == "" || mindate == '' || maxdate == '')
			{
				...
				if(mindate == '')
				{
					$('[name="mindate"]').addClass('error');
				}
				if(maxdate == '')
				{
					$('[name="maxdate"]').addClass('error');
				}
			...
			else if(mindate >= maxdate)
			{
				...
				$('[name="maxdate"]').removeClass('valid');
				$('[name="maxdate"]').addClass('error');
			}
			else
			{
				...
				$('#formsearch').attr('action', '{{ route("report.concentrated.result") }}').submit();
				// $.ajax(
				// {
				// 	type  : 'get',
