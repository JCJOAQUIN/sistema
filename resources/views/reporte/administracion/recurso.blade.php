					...
					<div class="float-right">
						@component('components.labels.label')
							@component('components.buttons.button',['variant' => 'success'])
								@slot('attributeEx') 
									type=submit 
									formaction={{ route('report.resource.excel') }} @endslot
								@slot('label')
					...
					<div class="float-right">
						@component('components.labels.label')
							@component('components.buttons.button',['variant' => 'success'])
								@slot('attributeEx') 
									type=submit 
									formaction={{ route('report.resource.excelwg') }} @endslot
								@slot('label')
				...
				foreach($requests as $request)
				{
					...
					[
						...
						[
							"content" =>
							[
								"label" => "$".number_format($request->resource->first()->total,2)
							]
						],
						[
							"content" =>
	...
	<script type="text/javascript"> 
		$(document).ready(function()
		{
			...
			$(document).on('click','[data-toggle="modal"]', function()
			{
				...
				{
					type : 'get',
					url  : '{{ route('report.resource.detail') }}',
					data : {'folio':folio},
