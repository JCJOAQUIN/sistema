...
@section('data')
	@component('components.forms.form', ["attributeEx" =>"method=\"POST\" id=\"container-alta\" action=\"".route('tickets.solve.assigned', $ticket->idTickets)."\"", "methodEx" => "PUT", "files" => true])
		...
		<div class="w-full grid grid-cols-1 md:flex gap-2 justify-items-center md:justify-center items-center mt-8">
			@if ($ticket->idStatusTickets == 1 || $ticket->idStatusTickets == 2 || $ticket->idStatusTickets == 3)
				@component('components.buttons.button', ["variant" => "primary", "attributeEx" => "type=\"submit\" name=\"save\" value=\"RESPONDER\"", "label" => "RESPONDER"]) @endcomponent
			@endif
			@php
