...
@section('data')
	@component('components.forms.form', ["attributeEx" => "method=\"POST\" id=\"container-alta\" action=\"".route('nomina.authorization.update', $request->folio)."\"", "methodEx" => "PUT"])
		<div class="sm:text-center text-left my-5">
			A continuación podrá verificar la información de la solicitud antes de continuar con el proceso:
		</div>
		@php
		 	$departmenId = $request->idDepartment == 11 ? 'Obra' : 'Administrativa';
			$modelTable = 
			[
				["Folio:",			$request->folio],
				...
				["Categoría:",		$departmenId." - ".$request->nominasReal->first()->typeNomina()],
			];
			switch($request->nominasReal->first()->idCatTypePayroll)
			{
				case('001'):
					$modelTable[] = ["Rango de fechas:", Carbon\Carbon::createFromFormat('Y-m-d',$request->nominasReal->first()->from_date)->format('d-m-Y')." - ".Carbon\Carbon::createFromFormat('Y-m-d',$request->nominasReal->first()->to_date)->format('d-m-Y')];					
					$modelTable[] = ["Periodicidad:", App\CatPeriodicity::find($request->nominasReal->first()->idCatPeriodicity)->description];
				break;
				case('002'):	
				break;
				case('003'):
					$modelTable[] = ["Fecha de baja:", Carbon\Carbon::createFromFormat('Y-m-d',$request->nominasReal->first()->down_date)->format('d-m-Y')];
				break;
				case('004'):
					$modelTable[] = ["Fecha de baja:", Carbon\Carbon::createFromFormat('Y-m-d',$request->nominasReal->first()->down_date)->format('d-m-Y')];
				break;
				...
				break;
				case('006'):
					$modelTable[] = ["PTU por pagar:", '$ '.number_format($request->nominasReal->first()->ptu_to_pay,2)];	
				break;
			}
		@endphp
		@component("components.templates.outputs.table-detail", ["title" => "Detalles de la Solicitud", "modelTable" => $modelTable, "classEx" => "mb-4"]) @endcomponent
		@component('components.labels.title-divisor') Lista de Empleados <span class="help-btn" id="help-btn-add-employee"> @endcomponent
			...
			@switch($request->nominasReal->first()->idCatTypePayroll)
				@case('001')
					...
					@php
						...
						{
							$dataWorker = $n->workerData->first()->paymentWay == 1 && $n->employee->first()->bankData()->where('visible',1)->exists() ? $n->employee->first()->bankData->where('visible',1)->last()->id : '';
		...
		@else
			...
			@php
				...
				$modelHead	= [
					[
						["value" => "Nombre del Empleado", "class" => "sticky inset-x-0"],
						["value" => "Tipo"],
		...
		@if($request->nominasReal->first()->type_nomina == 3)
			...
			@endcomponent
		@endif
		@if($request->idCheckConstruction != "" || $request->idCheck != "")
			@component('components.labels.title-divisor') DATOS DE REVISIÓN @endcomponent
			@php
				$construction = '';
				if($request->idCheckConstruction != "")
				{
					$construction = $request->constructionReviewedUser->name.' '.$request->constructionReviewedUser->last_name.' '.$request->constructionReviewedUser->scnd_last_name;
				}
				else
				{
					$construction = "---";
				...
				if($request->idCheck != "")
				{
					$check = $request->reviewedUser->name.' '.$request->reviewedUser->last_name.' '.$request->reviewedUser->scnd_last_name;
				}
				else
				{
					$check = '---';
		...
		<div class="w-full mt-4 grid grid-cols-1 md:flex gap-2 justify-items-center md:justify-center items-center mb-6">
			@component('components.inputs.input-text')
				@slot('attributeEx')
					type="hidden" name="decline_status" value="7"
				@endslot
			@endcomponent
			...
			@component('components.buttons.button', [ 
				...
				@slot('attributeEx')
					type="submit" name="decline" value="RECHAZAR SOLICITUD" formaction="{{ route('nomina.decline',["id" => $request->folio, "submodule_id" => 170]) }}"
				@endslot
				RECHAZAR SOLICITUD
			@endcomponent
		</div>
	...
	$(document).ready(function()
	{
		...
		.on('click','.btn-edit-employee',function()
		{
			...
			$.ajax(
			{
				...
				{
					$('#myModal .modal-body').show().html(data);
					$('.employee_amount, .employee_extra, .employee_discount').numeric({ negative : false });
		...
		.on('click','#add-bank',function()
		{
			...
			else if(bankid.length>0)
			{
				...
				else if(clabe != "" && ($(this).parents('.tr_bank').find(".clabe").hasClass("error") || clabe.length!=18))
				{
					swal("", "Por favor, debe ingresar 18 dígitos de la CLABE.", "error");
					$(this).parents('.tr_bank').find(".clabe").addClass("error");
				}
				else if(card != "" && ($(this).parents('.tr_bank').find(".card").hasClass("error") || card.length!=16))
				{
					swal("", "Por favor, debe ingresar 16 dígitos del número de tarjeta.", "error");
					$(this).parents('.tr_bank').find(".card").addClass("error");
				}
				else if(account != "" && ($(this).parents('.tr_bank').find(".account").hasClass("error") || (account.length>15 || account.length<5)))
				{
					swal("", "Por favor, debe ingresar entre 5 y 15 dígitos del número de cuenta bancaria.", "error");
					$(this).parents('.tr_bank').find(".account").addClass("error");
				}
				else if($(this).parents('.tr_bank').find('.card').hasClass('error') == false || $(this).parents('.tr_bank').find('.clabe').hasClass('error') == false || $(this).parents('.tr_bank').find('.account').hasClass('error') == false)
				{
					flag = false;
					$('#banks-body .tr_body').each(function()
					{
						name_account 	= $(this).find('.modal_account').val();
						name_clabe		= $(this).find('.modal_clabe').val();
						name_card		= $(this).find('.modal_card').val();
						...
						if(account != "" && name_account != "" && account == name_account)
						{
							...
							[
								...
								[
									...
									[
										...
										"classEx"		=> "clabeclass modal_clabe"
									]
								]
							],
							[
								...
								[
									...
									[
										...
										"classEx"		=> "accountclass modal_account"
									]
								]
							],
							[
		...
		@endif
		.on('click','[name="senddata"]',function()
		{
			validationEditEmployee();
		})
