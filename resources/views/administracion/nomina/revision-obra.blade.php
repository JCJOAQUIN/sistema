...
@section('data')
	@component('components.forms.form', ["attributeEx" => "method=\"POST\" id=\"container-alta\" action=\"".route('nomina.constructionreview.update', $request->folio)."\"", "methodEx" => "PUT"])
		<div class="sm:text-center text-left my-5">
			A continuación podrá verificar la información de la solicitud antes de continuar con el proceso:
		</div>
		@php
			$departament = $request->idDepartment == 11 ? 'Obra' : 'Administrativa';
			$modelTable = [
				["Folio",			$request->folio],
				...
				["Categoría:",		$departament.' - '.$request->nominasReal->first()->typeNomina()],
			];
			...
			@switch($request->nominasReal->first()->idCatTypePayroll)
				@case('001')
					...
					@php
						...
						{
							$employeeAccount = $n->workerData->first()->paymentWay == 1 && $n->employee->first()->bankData()->where('visible',1)->exists() ? $n->employee->first()->bankData->where('visible',1)->last()->id : '';
							$body = [ "classEx" => "tr_payroll",
								[
									...
									[
										...
										[
											"kind" 			=> "components.inputs.input-text",
											"attributeEx"	=> "type=\"hidden\" name=\"idemployeeAccount[]\" value=\"".$employeeAccount."\"",
		...
		<div class="w-full mt-4 grid grid-cols-1 md:flex gap-2 justify-items-center md:justify-center items-center mb-6">
			@component('components.inputs.input-text')
				@slot('attributeEx')
					type="hidden" name="decline_status" value="16"
				@endslot
			@endcomponent
			...
			@component('components.buttons.button', [ 
				...
				@slot('attributeEx')
					type="submit" name="decline" value="RECHAZAR SOLICITUD" formaction="{{ route('nomina.decline',["id" => $request->folio, "submodule_id" => 169]) }}"
				@endslot
				RECHAZAR SOLICITUD
			@endcomponent
		</div>
...
<script>
	$(document).ready(function()
	{
		...
		.on('click','.btn-edit-user',function()
		{
			...
			$(this).parent().find('.selectProvider').val(null).trigger('change');
		})
		...
		@switch($request->nominasReal->first()->idCatTypePayroll)
			@case('001')
				...
				.on('change','input[name="salary_imss"],input[name="salary_infonavit"],input[name="salary_fonacot"],input[name="salary_loan_retention"],input[name="salary_isrRetentions"],input[name="salary_other_retention_amount"]',function()
				{
					...
					total = Number(salary_imss)+Number(salary_infonavit)+Number(salary_fonacot)+Number(salary_loan_retention)+Number(salary_isrRetentions)+Number(salary_other_retention);
			...
			@case('003')
				...
				.on('change','input[name="liquidation_isr"]',function()
				{
					liquidation_isr = $('input[name="liquidation_isr"]').val();
					$('input[name="liquidation_totalRetentions"]').val(liquidation_isr);
