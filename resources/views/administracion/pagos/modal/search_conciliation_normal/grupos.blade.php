...
@php
	...
	$retentions		= 0;
	$modelBody		= [];
	foreach($req->groups->first()->detailGroups as $detail)
	{
		$taxesConcept=0;
...
@php
    ...
    $modelTable = 
    [
        ["label" => "Subtotal: ", "inputsEx" => [["kind"	=>	"components.labels.label",	"label"	=>	"$ ".number_format($req->groups->first()->subtotales,2,'.',','), "classEx" => "my-2"]]],
		...
		["label" => "IVA: ", "inputsEx" => [["kind"	=>	"components.labels.label",	"label"	=>	"$ ".number_format($req->groups->first()->tax,2,'.',','), "classEx" => "my-2"]]],
		["label" => "TOTAL: ", "inputsEx" => [["kind"	=>	"components.labels.label",	"label"	=>	"$ ".number_format($req->groups->first()->amount,2,'.',','), "classEx" => "my-2"]]],
    ];
@endphp
@component("components.templates.outputs.form-details",[
	...
	@if($req->groups->first()->notes != "")
		@slot('textNotes')
		 {{ htmlentities($req->groups->first()->notes) }}
		@endslot
	@endif
@endcomponent
