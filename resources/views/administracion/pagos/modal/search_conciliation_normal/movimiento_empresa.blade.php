...
@php 
	...
	$requestAccountOrigin  = App\Account::find($req->movementsEnterprise->first()->idAccAccOrigin);
	$requestAccountDestiny = App\Account::find($req->movementsEnterprise->first()->idAccAccDestiny);
	$modelTable = 
	[
		["Folio:", $req->folio],
		["Título y fecha:", htmlentities($req->movementsEnterprise->first()->title)." - ".Carbon\Carbon::createFromFormat('Y-m-d', $req->movementsEnterprise->first()->datetitle)->format('d-m-Y')],
		["Fiscal:", $req->taxPayment == 1 ? "Si" : "No" ],
		["Solicitante:", $requestUser->fullname()],
		["Elaborado por:", $elaborateUser->fullname()],
		["Empresa Origen:", App\Enterprise::find($req->movementsEnterprise->first()->idEnterpriseOrigin)->name],
		...
		["Empresa Destino", App\Enterprise::find($req->movementsEnterprise->first()->idEnterpriseDestiny)->name],
...
@php
	$modelTable =
	[
		"Tipo de moneda"    =>  $req->movementsEnterprise->first()->typeCurrency,
		"Fecha de pago"     => Carbon\Carbon::createFromFormat('Y-m-d', $req->movementsEnterprise->first()->paymentDate)->format('d-m-Y'),
		"Forma de pago"     => $req->movementsEnterprise->first()->paymentMethod->method,
		"Importe a pagar"   => "$ ".number_format($req->movementsEnterprise->first()->amount,2),
	];
@endphp
...
@if(count($req->movementsEnterprise->first()->documentsMovements)>0)
   	@php
		$modelHead =
		[
			[
		...
		$modelBody = [];
		foreach($req->movementsEnterprise->first()->documentsMovements as $doc)
		{
			$modelBody[] = 
			[
				"classEx" => "tr",
