...
<div id="container-cambio" class="div-search">
	{!! Form::open(['route' => 'procurement-purchases.warehouse', 'method' => 'GET', 'id'=>'formsearch']) !!}
		...
		<table class="table table-striped">
			...
			@foreach($requests as $request)
				<tr>
					...
					<td>{{ $date  }}</td>
					<td>
						<a alt="Editar Orden" title="Editar Orden" href="{{ route('procurement-purchases.warehouse-create',$request->id) }}" class='btn follow-btn'>
...
<script type="text/javascript"> 
	$(document).ready(function()
	{
		...
		$(document).on('click','.export',function(e)
		{
			...
			attr = $(this).attr('href');
			swal({
				text: "Solo se descargarán las ordenes 'Autorizadas'",
				icon: "warning",
				buttons: ["Cancelar","OK"],
			})
			.then((isConfirm) =>
