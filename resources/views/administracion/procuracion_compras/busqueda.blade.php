...
<div id="container-cambio" class="div-search">
	{!! Form::open(['route' => 'procurement-purchases.purchase-follow', 'method' => 'GET', 'id'=>'formsearch']) !!}
					...
					<p>
						<select title="Estado de Solicitud" name="status[]" class="js-status" multiple="multiple" style="width: 98%; max-width: 150px;">
							@foreach(App\StatusRequest::whereIn('idrequestStatus',[24,25,26])->get() as $stat)
								<option value="{{ $stat->idrequestStatus }}" @if(isset($status) && in_array($stat->idrequestStatus,$status)) selected="selected" @endif>{{ $stat->description }}</option>
							@endforeach
						</select>
					</p>
				</div>
...
<br>
@if(count($requests) > 0)
	<div style='float: right'><a class="btn btn-orange export" href="{{ route('procurement-purchases.purchase-export') }}"><i class="fas fa-file-archive"></i> Exportar Ordenes</a></div>
	{!! Form::close() !!}
				...
				<tr>
					...
					<td>
						@if($request->status == 24)
							<a alt="Editar Orden" title="Editar Orden" href="{{ route('procurement-purchases.purchase-edit',$request->id) }}" class='btn follow-btn'>
								<i class="fas fa-pencil-alt"></i>
							</a>
						@else
							<a alt="Ver Orden" title="Ver Orden" href="{{ route('procurement-purchases.purchase-view',$request->id) }}" class='btn follow-btn'>
					...
					<td>
						@if($request->status == 24)
							<a alt="Cancelar Orden" title="Cancelar Orden" href="{{ route('procurement-purchases.purchase-cancel',$request->id) }}" class='btn follow-btn'>
