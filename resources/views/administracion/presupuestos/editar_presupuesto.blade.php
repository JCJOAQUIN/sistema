...
@section('data')
	@if ($budgetUpload->status != 'Subiendo')
		@component("components.forms.form", ["attributeEx" => "method=\"POST\" action=\"".route('budget.finish')."\" id=\"container-alta\" method=\"POST\"", "files" => "true"])
			...
			<div class="w-full grid grid-cols-1 md:flex gap-2 justify-items-center md:justify-center items-center mt-12">
				@component('components.buttons.button', ["variant"	=>	"red", "buttonElement" => "a"])
					@slot('attributeEx')
						href="{{ route('budget.delete', $budgetUpload->id) }}"
					@endslot
					@slot('label')
						ELIMINAR
					@endslot
...
{{-- 
@if ($budgetUpload->status != 'Subiendo')
	{!! Form::open(['route' => 'budget.finish', 'method' => 'POST', 'id' => 'container-alta','files'=>true]) !!}
		...
		<br>
		<center>
			<button type="submit" formaction="{{ route('budget.delete') }}" class="btn btn-red">ELIMINAR</button>
...
$.validate(
{
	...
	onSuccess : function($form)
	{
		...
		.on('click','.send-edit',function(){
			...
			return (id, code, concept, date, ammount, price, n_import, incidence, measurement);
		...
		function paginate_arts(page =undefined,firstSearch = false) {
			...
			{
				type : 'get',
				url  : '{{ route("budget.paginate.arts") }}',
				data : {
					'page':page,
				...
				success : function(response)
				{
					...
					newStatus = response.BudgetUploads.status
