...
@section('data')
	@if ($budgetUpload->status != 'Subiendo')
		@component("components.forms.form", ["attributeEx" => "method=\"POST\" action=\"".route('UnitPrices.finish')."\" id=\"container-alta\"", "files" => true])
			...
			<div class="w-full grid grid-cols-1 md:flex gap-2 justify-items-center md:justify-center items-center mt-12">
				@component('components.buttons.button', ["variant"	=>	"red", "buttonElement" => "a"])
					@slot('attributeEx')
						href="{{ route('UnitPrices.delete', $budgetUpload->id) }}"
					@endslot
					@slot('label')
						ELIMINAR
					@endslot
...
@section('scripts')
	...
	<script type="text/javascript">
		...
		{
			id          = $(this).parent('.id-row').find('.idd ').val().trim();
		...
		.on('click','.send-edit',function()
		{
			...
			{
				type       : 'post',
				url        : '{{ route("UnitPrices.article.edit") }}',
				data       : formData,
				contentType: false,
				processData: false,
		...
		function paginate_arts(page =undefined,firstSearch = false)
		{
			...
			{
				type : 'get',
				url  : '{{ route("UnitPrices.paginate.arts") }}',
				data :
				{
					'page':page,
