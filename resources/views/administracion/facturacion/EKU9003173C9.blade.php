					...
					<table class="request-info no-border centered-table">
						<tbody>
							...
							<tr>
								...
								<td @if($bill->type == 'N') colspan="2" @endif>
									<span class="normal">{{$bill->postalCode}} - {{ $bill->postal_code->state }}, {{$bill->expeditionDateCFDI}}</span>
								</td>
							</tr>
							@if($bill->stampDate != '')
								<tr>
										...
										<table class="table table-bordered">
											...
											<tbody>
												<tr>
													...
													<td>{{$d->quantity}}</td>
													<td>{{$d->keyUnit}}</td>
													<td>{{number_format($d->value,6)}}</td>
													<td>{{number_format($d->amount,6)}}</td>
													<td>{{number_format($d->discount,6)}}</td>
												</tr>
												<tr>
																...
																<tbody>
																	@foreach($d->taxesRet as $r)
																		<tr>
																			...
																			<td>{{$r->quota}}</td>
																			<td>{{$r->quotaValue}}</td>
																			<td>{{number_format($r->amount,6)}}</td>
																		</tr>
																	@endforeach
																</tbody>
															</table>
						...
						<tbody>
							<tr>
								<td>
									...
									@endphp
									{{ strtoupper(str_replace('­cientos','cientos',$letter)) }} {{ $totalTemp[1] }}/100 {{$bill->currency}}
								</td>
							</tr>
						</tbody>
					</table>
