				...
				<div class="block-info">
					<table class="centered-table proyecta-border">
						<tbody>
							<tr>
								<td width="70%">
									<div class="request-info" style="width: 80%;margin: 0 0 0 auto;">
										<br>
										<p><strong>RFC emisor:</strong> {{$bill->rfc}}</p>
										<p><strong>Nombre emisor:</strong> {{$bill->businessName}}</p>
										@if ($bill->issuer_address != '')
										<p style="padding-left: .5em;">{{$bill->issuer_address}}</p>
										@endif
										<p><strong>RFC receptor:</strong> {{$bill->clientRfc}}</p>
										<p><strong>Nombre receptor:</strong> {{$bill->clientBusinessName}}</p>
										@if ($bill->receiver_address != '')
										<p style="padding-left: .5em;">{{$bill->receiver_address}}</p>
										@endif
										<br>
									</div>
								</td>
								<td width="30%" style="vertical-align: middle;">
									...
									@endphp
									<img src="{{ $base64 }}" width="150" style="margin-top: 20px;">
									<p><br></p>
								</td>
							</tr>
						</tbody>
					...
					<table class="request-info no-border centered-table">
						<tbody>
							...
							<tr>
								...
								<td @if($bill->type == 'N') colspan="2" @endif>
									<span class="normal">@if($bill->useBill != ''){{$bill->cfdiUse->description}} @endif</span>
								</td>
							</tr>
							<tr>
								<th @if($bill->type == 'N') colspan="2" @endif>
							...
							<tr>
								...
								<td @if($bill->type == 'N') colspan="2" @endif>
									<span class="normal">{{$bill->postalCode}} @if($bill->postal_code()->exists()) - {{ $bill->postal_code->state }} @endif, {{$bill->expeditionDateCFDI}}</span>
								</td>
							</tr>
							@if($bill->stampDate != '')
								<tr>
								...
								<tr>
									<td>
										<table class="table table-bordered">
											...
											<tbody>
												<tr>
													<td>{{$d->keyProdServ}} - {{ $d->cfdi_product->description }}</td>
													<td>{{$d->quantity}}</td>
													<td>{{$d->keyUnit}}</td>
						...
						<tbody>
							<tr>
								<td>
									...
									@endphp
									{{ strtoupper(str_replace('­cientos','cientos',$letter)) }} @if($bill->currency == 'MXN') PESOS @elseif($bill->currency == 'USD') DÓLARES @elseif($bill->currency != '') {{strtoupper($bill->cfdiCurrency->description)}} @endif {{ $totalTemp[1] }}/100 {{ $bill->currency }}
								</td>
							</tr>
						</tbody>
					</table>
									...
									<table class="request-info table table-bordered proyecta-payment">
										<tbody>
											...
											@if($bill->conditions != null)
												<tr>
													<th align="left">Condiciones de pago</th>
													<td align="left">{{$bill->conditions}}</td>
												</tr>
											@endif
										</tbody>
									</table>
								...
								<tr>
									...
									<td>
										<table class="table table-borderless request-info no-border centered-table">
											...
											@if($bill->paymentComplement->first()->exchange != '')
												<tr>
													<th align="left">Tipo de cambio de pago</th>
													<td align="right">{{$bill->paymentComplement->first()->exchange}}</td>
												</tr>
											@endif
												<tr>
