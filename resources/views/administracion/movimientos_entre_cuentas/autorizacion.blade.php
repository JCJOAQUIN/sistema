				...
				<div class="col-span-2">
					...
					@php
						...
						foreach(App\RequestKind::orderName()->whereIn('idrequestkind',[11,12,13,14,15])->get() as $k)
						{
							...
							{
								$options	=	$options->concat([["value"	=>	$k->idrequestkind, "description"	=>	$k->kind, "selected"	=>	"selected"]]);
							}
							else
							{
								$options	=	$options->concat([["value"	=>	$k->idrequestkind, "description"	=>	$k->kind]]);
							}
						}
					@endphp
			...
			@if(count($requests) > 0)
				@slot('export')
					@component('components.inputs.input-text')
						@slot('attributeEx')
							type="hidden" value="autorización" name="type"
						@endslot
					@endcomponent
					<div class="flex flex-row justify-end">
						@component('components.buttons.button', ['variant' => 'success'])
							...
							@endslot
							@slot('attributeEx')
								type='submit' formaction="{{ route('movements-accounts.authorization.excel') }}" formmethod="GET"
							@endslot
						@endcomponent
					</div>
				@endslot
				...
				[
					...
					[
						[
							...
							"attributeEx"	=>	" title=\"Editar Solicitud\" href=\"".route('movements-accounts.authorization.edit',$request->folio)."\""
						]
					]
				],
			];
