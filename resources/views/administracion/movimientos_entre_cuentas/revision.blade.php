		...
		@component('components.forms.searchForm', ["attributeEx" => "id=\"formsearch\"", "values" => $values, "hidden" => $hidden])
			@slot('contentEx')
				<div class="col-span-2">
					...
					@php
						$optionsEnterprises	=	collect();
						foreach(App\RequestKind::orderName()->whereIn('idrequestkind',[11,12,13,14,15])->get() as $k)
						{
							if(isset($kind) && $kind == $k->idrequestkind)
							{
								$optionsEnterprises	=	$optionsEnterprises->concat([["value"	=>	$k->idrequestkind,	"description"	=>	$k->kind,	"selected"	=>	"selected"]]);
							}
							else 
							{
								$optionsEnterprises	=	$optionsEnterprises->concat([["value"	=>	$k->idrequestkind,	"description"	=>	$k->kind]]);
							}
						}
					@endphp
			...
			@if(count($requests) > 0)
				@slot('export')
					@component('components.inputs.input-text')
						@slot('attributeEx')
							type="hidden" value="revisión" name="type"
						@endslot
					@endcomponent
					@component('components.buttons.button', ['variant' => 'success'])
						...
						@endslot
						@slot('attributeEx')
							type='submit' formaction="{{ route('movements-accounts.review.excel') }}" formmethod="GET"
						@endslot
					@endcomponent
				@endslot
			@endif
					...
					[
						...
						[
							[
								...
								"attributeEx"	=>	" title=\"Editar Solicitud\" href=\"".route('movements-accounts.review.edit',$request->folio)."\""
							]
						]
					],
				];
