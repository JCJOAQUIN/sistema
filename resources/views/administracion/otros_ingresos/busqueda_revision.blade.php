...
<div id="container-cambio" class="div-search">
	{!! Form::open(['route' => 'other-income.review', 'method' => 'GET', 'id'=>'formsearch']) !!}
		<center>
...
<br>
@if(count($requests) > 0)
	<div style='float: right'><label class='label-form'>Exportar a Excel <label><button class='btn btn-green export' type='submit'  formaction="{{ route('other-income.excel-review') }}"><span class='icon-file-excel'></span></button></div>
	{!! Form::close() !!}
		...
		<table class="table">
			...
			@foreach($requests as $request)
				...
				<tr>
					...
					<td>{{ $date  }}</td>
					<td>
						<a alt="Editar Solicitud" title="Editar Solicitud" href="{{ route('other-income.review.show',$request->folio) }}" class='btn follow-btn'><span class='icon-pencil'></span></a>
					</td>
				</tr>
			@endforeach
		</table>
