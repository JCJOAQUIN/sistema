...
@section('data')
	@php
		...
		$taxes	= 0;
	@endphp
	@component('components.forms.form', ["attributeEx" => "method=\"POST\" id=\"container-alta\" action=\"".route('expenses.store')."\"", "files" => true])
		...
		@component('components.containers.container-form')
			<div class="col-span-2">
				...
				@component('components.inputs.input-text')
					...
					@slot('attributeEx')
						...
						@if(isset($requests)) value="{{ $requests->expenses->first()->title }}" @endif
					@endslot
				@endcomponent
			</div>
			@php
				...
				if(isset($requests))
				{
					$newDate = isset($requests->expenses->first()->datetitle) ? Carbon\Carbon::createFromFormat('Y-m-d',$requests->expenses->first()->datetitle)->format('d-m-Y') : '';
				}
			@endphp
			<div class="col-span-2">
				...
				@component('components.inputs.input-text')
					...
					@slot('attributeEx')
						...
						readonly="readonly"
						name="datetitle" 
						@if(isset($requests)) value="{{ $newDate }}" @endif 	
					@endslot
				@endcomponent
			</div>
			<div class="col-span-2">
			...
			<div class="col-span-2 js-resources-container">
				...
				@php
					...
					{  
						$resourceList = array();
						$key = 0;
						foreach(App\RequestModel::where('kind',8)->whereIn('status',[5,10,11,12])->where('idRequest',$requests->idRequest)->get() as $value)
						{ 
							if($value->resource->first()->expensesRequest->count()==0)
							{
								$resourceList[$key]['folio'] = $value->folio;
								$resourceList[$key]['nombre'] = $value->resource->first()->title;
								$key++;
							}
							else 
							{
						...
						{
							if($requests->expenses->first()->resourceId == $list['folio'])
							{
								$optionResources[] = ["value" => $list['folio'], "description" => $list['folio'].' - '.$list['nombre'], "selected" => "selected"];
							}
							else
							{
								$optionResources[] = ["value" => $list['folio'], "description" => $list['folio'].' - '.$list['nombre']];
							}
						}
					}
				@endphp
				@component('components.inputs.select', ["options" => $optionResources])
					@slot('attributeEx')
						multiple="multiple" 
						name="resources_id"
						data-validation="required"
		...
		@endcomponent
		@if(isset($requests) && $requests->expenses->first()->resourceId != "")
			<div id="dates" class="mt-4">
				@php
					$modelTable =
					[
						["Empresa:", 		$requests->requestEnterprise->name],
						["Dirección:",		$requests->requestDirection->name ],
						["Departamento:",	$requests->requestDepartment->name],
						["Proyecto:",		$requests->requestProject->proyectName],
						["Código WBS:", 	isset($requests->wbs) ? $requests->wbs->code_wbs : "---"],	 
						["Código EDT:", 	isset($requests->edt) ? $requests->edt->fullName() : "---"]
					];
				@endphp
				...
				@component('components.inputs.input-text')
					@slot('attributeEx')
						type="hidden" name="enterprise_id" value="{{ $requests->requestEnterprise->id }}"
					@endslot
				@endcomponent
				@component('components.inputs.input-text')
					@slot('attributeEx')
						type="hidden" name="area_id" value="{{ $requests->requestDirection->id }}"
					@endslot
				@endcomponent
				@component('components.inputs.input-text')
					@slot('attributeEx')
						type="hidden" name="department_id" value="{{ $requests->requestDepartment->id }}"
					@endslot
					@endcomponent
				@component('components.inputs.input-text')
					@slot('attributeEx')
						type="hidden" name="project_id" value="{{ $requests->requestProject->idproyect }}"
					@endslot
				@endcomponent
				@isset($requests->wbs)
					@component('components.inputs.input-text')
						@slot('attributeEx')
							type="hidden" name="wbs_id" @isset($requests->wbs) value="{{$requests->wbs->id}}" @endisset
						@endslot
					@endcomponent
					@isset($requests->edt)
						@component('components.inputs.input-text')
							@slot('attributeEx')
								type="hidden" name="edt_id" @isset($requests->edt) value="{{$requests->edt->id}}" @endisset
							@endslot
						@endcomponent
					@endisset
				@endisset
		...
		@if(isset($requests))
			...
			@php
				...
				[
					[
						...
						"attributeButton" 	=> "type=\"radio\" name=\"method\" value=\"1\" id=\"accountBank\"".($requests->expenses->first()->idpaymentMethod == 1 ? " checked" : ""),
					],
					[
						"textButton" 		=> "Efectivo",
						"attributeButton" 	=> "type=\"radio\" name=\"method\" value=\"2\" id=\"cash\"".($requests->expenses->first()->idpaymentMethod == 2 ? " checked" : ""),
					],
					[
						"textButton" 		=> "Cheque",
						"attributeButton" 	=> "type=\"radio\" name=\"method\" value=\"3\" id=\"checks\"".($requests->expenses->first()->idpaymentMethod == 3 ? " checked" : ""),
					],								
				];
			@endphp
			...
			@component('components.inputs.input-text')
				@slot('attributeEx')
					...
					value="@foreach($requests->expenses as $expense){{ $expense->idUsers }}@endforeach"
				@endslot
				@slot('classEx')
					employee_number
				@endslot
			@endcomponent
			<div class="resultbank mb-4 @if($requests->expenses->first()->idpaymentMethod == 1) @else hidden @endif">
				@component('components.labels.title-divisor') SELECCIONE UNA CUENTA: @endcomponent
				@php
					$body		=[];
					$modelBody	=[];
					$modelHead	=["Acción","Banco","Número de tarjeta","CLABE","Número de cuenta"];
						...
						foreach(App\Employee::join('banks','employees.idBanks','banks.idBanks')->where('employees.idUsers',$expense->idUsers)->get() as $bank)
						{
							...
							$body = [ "classEx" => $class,
								...
								[
									...
									[
										[
											"label" => isset($bank->description) ? $bank->description : '---'
										]
									]
								],
								[
			...
			@component('components.containers.container-form')
				<div class="col-span-2">
					...
					@component('components.inputs.input-text')
						@slot('attributeEx')
							type="text"
							name="reference" 
							value="@foreach($requests->expenses as $expense) {{ $expense->reference }} @endforeach" 
				...
				<div class="col-span-2">
					...
					@php
						...
						{
							if($v == $requests->expenses->first()->currency)
							{
		...
		@else
			...
			@php
				...
				[
					...
					[
						"textButton" 		=> "Efectivo",
						"attributeButton" 	=> "type=\"radio\" name=\"method\" value=\"2\" id=\"cash\" checked=\"checked\"",
					],
					[
						"textButton" 		=> "Cheque",
			...
			@component('components.containers.container-form')
				...
				<div class="col-span-2">
					...
					@php
						$optionCurrency = [];
						$optionCurrency[] = ["value" => "MXN", "description" => "MXN"];
						$optionCurrency[] = ["value" => "USD", "description" => "USD"];
						$optionCurrency[] = ["value" => "EUR", "description" => "EUR"];
						$optionCurrency[] = ["value" => "Otro", "description" => "Otro"];						
					@endphp
					@component('components.inputs.select', ["options" => $optionCurrency])
						@slot('attributeEx')
							multiple="multiple"
			...
			<div>
				@component('components.buttons.button-approval')
					@slot('attributeEx')
						...
						id="doc_exist" 
						value="exist" 
						@isset($requests) @if($requests->expenses->first()->resourceId != "") checked="checked" @endif @endisset
					@endslot
					@slot('id')
						id="c"
					@endslot
		...
		<div class="mb-4 @if(isset($requests)) @if($requests->expenses->first()->resourceId != "")  block @else hidden @endif @else hidden @endif" id="docs_exist">
			<div id="documents-resource" class="@if(isset($requests)) @if($requests->expenses->first()->resourceId != "")  block @else hidden @endif @else hidden @endif">
				...
				@php
					...
					if(isset($requests))
					{
						if($requests->expenses->first()->resourceId != "")
						{
							foreach(App\Resource::where('idFolio',$requests->expenses->first()->resourceId)->get() as $resource)
							{
										...
										[
											...
											[
												...
												[
													...
													"attributeEx"	=> "name=\"idresourcedetail-table\" type=\"hidden\" value=\"".$resourceDetail->idresourcedetail."\""
												],
												[
													"kind" 			=> "components.inputs.input-text",
													"classEx"		=> "concept-table",
													"attributeEx"	=> "name=\"concept-table\" type=\"hidden\" value=\"".$resourceDetail->concept."\""
												]
											]
										],
										[
											...
											[
												...
												[
													...
													"attributeEx" 	=> "name=\"account-table\" type=\"hidden\" value=\"".$resourceDetail->accountsReview->account.' '.$resourceDetail->accountsReview->description."\""
												],
												[
													"kind"			=> "components.inputs.input-text",
													"classEx"		=> "accountid-table",
													"attributeEx"	=> "name=\"accountid-table\" type=\"hidden\" value=\"".$resourceDetail->idAccAccR."\""
												]
											]	 
										],
										[
											...
											[
												...
												[
													...
													"attributeEx"	=> "name=\"amount-table\" type=\"hidden\" value=\"".$resourceDetail->amount."\""
												]
											]
										],
										[
			...
			@component("components.containers.container-form")	
				...
				<div class="col-span-2 md:col-span-4 grid grid-cols-1 md:grid-cols-2 gap-6" id="documents_exist"></div> 
		...
		<div id="docs_new" class="hidden">
			@component("components.containers.container-form")
				...
				<div id="documents_new" class="col-span-2 md:col-span-4 grid grid-cols-1 md:grid-cols-2 gap-6">
				</div>
				<div class="col-span-2 md:col-span-4 space-x-2 text-center md:text-left">
					@component('components.buttons.button', ["variant" => "warning"])
						@slot('attributeEx')
		...
		<div> 
			@php
				...
				if(isset($requests))
				{
					if($requests->status == 6 || $requests->status == 7)
					{
						foreach($requests->expenses->first()->expensesDetail as $key=>$expensesDetail)
						{
							...
							[
								...
								[
									...
									[
										...
										[
											"kind" 			=> "components.inputs.input-text",
											"attributeEx"	=> "type=\"hidden\" name=\"t_fiscal[]\" value=\"".$paymentDetail."\"",
											"classEx"		=> "t_fiscal"
										]
									]
								],
							...
							if($expensesDetail->documents()->exists())
							{
								foreach($expensesDetail->documents as $doc)
								{
									...
									$docsExpenses .= view('components.inputs.input-text',[
										"attributeEx"	=> "type=\"hidden\"  name=\"t_fiscal_folio".$docs."[]\" value=\"".$doc->fiscal_folio."\"",
										"classEx"		=> "num_fiscal_folio"	
									])->render();
									$docsExpenses .= view('components.inputs.input-text',[
										"attributeEx"	=> "type=\"hidden\"  name=\"t_ticket_number".$docs."[]\" value=\"".$doc->ticket_number."\"",
					...
					[
						...
						[
							[
								...
								"classEx" 	=> "my-2 general-class totalResourceLabel"
							],
							[
								"kind"			=> "components.inputs.input-text",
								"classEx" 		=> "removeselect totalResource",	
								"attributeEx" 	=> "type=\"hidden\" name=\"totalResource\" value=\"".number_format($total,2)."\""
							]
						]
					],
					[
					...
					[
						...
						[
							[
								"kind" 		=> "components.labels.label",
								"label" 	=> $varIvaLabel,
								"classEx" 	=> "my-2 general-class ivaTotalLabel"
							],
							[
								"kind"			=> "components.inputs.input-text",
								"classEx" 		=> "removeselect ivaTotal",	
					...
					[
						...
						[
							[
								...
								"classEx"	=> "my-2 general-class labelAmount"
							],
							[
								"kind"			=> "components.inputs.input-text",
					...
					[
						...
						[
							[
								"kind" 		=> "components.labels.label",
								"label" 	=> "$ 0.00",
								"classEx" 	=> "my-2 general-class reintegroLabel"
							],
							[
								"kind"			=> "components.inputs.input-text",
								"attributeEx" 	=> "type=\"hidden\" id=\"reintegro\" name=\"reintegro\"",
								"classEx" 		=> "removeselect reintegro"
							]
						]
					],
					[
						...
						[
							[
								"kind" 		=> "components.labels.label",
								"label" 	=> "$ 0.00",
								"classEx" 	=> "my-2 general-class reembolsoLabel"
							],
							[
								"kind"			=> "components.inputs.input-text",
								"attributeEx" 	=> "type=\"hidden\" id=\"reembolso\" name=\"reembolso\"",
								"classEx" 		=> "removeselect reembolso"
							]
						]
					],
					[
		...
		<div class="w-full mt-4 grid grid-cols-1 md:flex gap-2 justify-items-center md:justify-center items-center mb-6">
			...
			@component('components.buttons.button', [
				...
				@slot('attributeEx')
					...
					id="save" 
					name="save" 
					formaction="{{ route('expenses.unsent') }}"
				@endslot
				@slot('classEx')
					save
				@endslot
...
<script>
 	$(document).ready(function()
	{
		...
		if (!$('select[name="resources_id"]').val()) 
		{
		...
		.on('click', '#doc_exist', function()
		{
			$('#docs_new').find('.input-all').val('');
			$('#docs_exist').find('.contentMoney_new').val('');
			$('#docs_new').find('.amount').val('');
			$('#docs_new').find('.additional_newName').val('');
			$('#docs_new').find('.additional_newAmount').val('');
			$('#docs_new').find('.js-accounts').val('').trigger('change');
			$('#docs_new').find('.fiscal').prop('checked',false);
			$('#docs_new').find('.iva').prop('checked',false);
			$('#docs_new').find('.iva_kind').prop('checked',false);
			$('#docs_new').find('.addiotional').prop('checked',false);
			$('#docs_new').find('.docs-p').remove();
			row = "";
			if(row != null)
			{
		...
		.on('click','.add',function()
		{
			...
			if ($('input[name="exist_new"]:checked').val() == "exist") 
			{
				...
				if($('[name="additional_exist"]:checked').val() == "si")
				{
					...
					{
						additionalTax = additionalTax + $(this).val();
						if($(this).val() == '')
						{
							cont = false;
				...
				$('.name_document').each(function(i,v)
				{
					if($(this).val()=='')
					{
						$(this).addClass('error');
						cont = false;
						swal('','Por favor seleccione los tipos de documentos','error');
					}
				});
				if(path != 0 && cont)
				{
					...
					function addTr(countConcept,amountAAtotal,idresourcedetail,concept,date,iva,ivaKind,fiscal,subtotal,account,accountText,path,ivaParam,ivaParam2,ivaCal,total,cont)
					{
						...
						$(".path_exist").each(function(i, v)
						{
							...
							row.find('.docValues').append($('<div class="nowrap"></div>').append($('.datepath_exist').get(i).value)
										.append($(buttonPDF).attr('title',pathName).attr("href", pathName))
			...
			else
			{
				...
				$('.name_document_new').each(function(i,v)
				{
					if($(this).val()=='')
					{
						cont = false;
						$(this).addClass('error');
						swal('Error','Por favor seleccione los tipos de documentos','error');
						return;
					}
				});
				...
				if (concept == ""  || date == 0 || subtotal == "" || account == "" || path == 0)
				{
					...
					{
						$('.path_new').addClass('error');
						swal('','Por favor de agregar un documento','error');
					}
					else
					{
						swal('', 'Por favor llene los campos necesarios', 'error');
					...
					if ($('.datepath_new').length > 0) 
					{
						...
						.done(function(data)
						{
							if (!flag) 
							{
								...
								{
									swal('','Error, el costo total del concepto debe ser mayor a cero','error');
								}
							}
						});
					}
		...
		.on('click','.btn-delete-form',function(e)
		{
			...
			.then((willClean) =>
			{
				if(willClean)
				{
					$('#body').html('');
					$('#documents').empty();
					$('.general-class').text('$ 0.00');
					$('.removeselect').val(null).removeClass('valid').trigger('change');
					form[0].reset();
				}
				else
				{
		...
		{
			swal('Ayuda','En este apartado hay dos opciones, una es seleccionar la opción "Existente" en caso de tomar los conceptos solicitados en la solicitud de asignación de recurso. La opción "Nuevo" es para agregar nuevos conceptos en caso de que se hayan realizado otros gastos que no estaban contemplados en la solicitud de asignación de recurso.','info');
		})
	...
	.on('change','.addiotional,.amount_exist,.iva,.iva_kind,.additional_existAmount,.fiscal', function()
	{
