...
@section('data')
	...
	{!! Form::open(['route' => ['payroll.review.update', $request->folio], 'method' => 'put', 'id' => 'container-alta']) !!}
		<div class="form-container">
			<center>
				<p>
		...
		<div id="aceptar">				
			<div class="form-container">
				...
				<textarea class="text-area" cols="90" rows="10" name="checkCommentA"></textarea>
			</div>
		</div>
		<div id="rechaza">
			<div class="form-container">
				<label class="label-form">Comentarios (opcional)</label>
				<textarea class="text-area" cols="90" rows="10" name="checkCommentR"></textarea>
			</div>
		</div>
		<center>
			<p>
	...
	$(document).ready(function()
	{
		...
		.on('change','.js-enterprisesR',function()
		{
			...
			$.ajax(
			{
				...
				success : function(data)
				{
					...
					{
						$('.js-accountsR').append('<option value='+d.idAccAcc+'>'+d.account+' - '+d.description+'</option>');
					});
				},
				error : function()
				{
		...
		.on('change','input[name="status"]',function()
		{
			if ($('input[name="status"]:checked').val() == "4") 
			{
				...
				$("#aceptar").slideToggle("slow").addClass('form-container').css('display','flex');

				$('.js-enterprisesR').select2(
				{
			...
			else if ($('input[name="status"]:checked').val() == "6") 
			{
				...
				$("#rechaza").slideToggle("slow").addClass('form-container').css('display','flex');
			}
		});
