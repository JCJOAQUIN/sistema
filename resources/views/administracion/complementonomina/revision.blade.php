...
@section('data')
		<div id="container-cambio" class="div-search">
			{!! Form::open(['route' => 'payroll.review', 'method' => 'GET', 'id'=>'formsearch']) !!}
	...
	<br>
@if(count($requests) > 0)
<div style='float: right'><label class='label-form'>Exportar a Excel <label><button class='btn btn-green export' type='submit'  formaction="{{ route('payroll.export.review') }}"><span class='icon-file-excel'></span></button></div>
{!! Form::close() !!}
<div class="table-responsive table-striped">
	...
	<tr>
		...
		@endphp 
		<td>{{ $date  }}</td>
		<td>{{ isset($request->requestEnterprise->name) ? $request->requestEnterprise->name : "Varias" }}</td>
		<td>{{ isset($request->accounts->account) ? $request->accounts->account.' '.$request->accounts->description : "Varias" }}</td>
		<td><a title="Editar Solicitud" href="{{ route('payroll.review.edit',$request->folio) }}" class='btn follow-btn'><span class='icon-pencil'></span></a></td>
...
<center>
	{{ $requests->appends(['folio' => $folio, 'name' => $name, 'mindate' => $mindate, 'maxdate' => $maxdate])->render() }}
</center><br><br><br>
@else
