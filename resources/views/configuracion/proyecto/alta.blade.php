...
@section('data')
	@component("components.forms.form", ["attributeEx" => "action=\"".route('project.store')."\" method=\"POST\" id=\"container-alta\""])
		...
		<div class="text-center">
			@component("components.buttons.button", ["variant" => "warning", "attributeEx" => "type=\"button\" id=\"mapToggle\"","classEx" => "location-closed", "label" => "<span class=\"icon-plus\"></span> Agregar ubicación"]) @endcomponent
		</div>
		...
		$(document).ready(function() 
		{
			...
			.on('click','.btn-delete-form',function(e)
			{
				...
				swal({
					...
					text		: "¿Confirma que desea limpiar el formulario? \n Esta acción descartará toda la información ingresada, incluyendo los datos de los Subproyectos.",
					icon		: "warning",
					buttons		: ["Cancelar","OK"],
					dangerMode	: true,
				})
				.then((willClean) =>
				{
					if(willClean)
					{
						...
						if($(".map-container").is(":visible"))
						{
							...
							$("#mapToggle").text('Agregar Ubicación').removeClass('bg-red-600').addClass('bg-orange-400 hover:bg-orange-300');
							$("#latitude").val('');
							$("#longitude").val('');
						}
