...
@section('data')
	@component("components.forms.form", ["attributeEx" => "action=\"".route('provider.update', $provider->idProvider)."\" method=\"POST\" id =\"container-alta\""])
		@slot("methodEx") PUT @endslot
		@component('components.labels.title-divisor') DATOS DEL PROVEEDOR @endcomponent
		@component("components.labels.subtitle") Para editar el proveedor es necesario colocar los siguientes campos: @endcomponent
		@component("components.containers.container-form")
			<div class="col-span-2">
				...
				@component("components.inputs.input-text")
					@slot("attributeEx")
						...
						data-validation-req-params = "{{ json_encode(array('oldReason'=>$provider->businessName)) }}"
			...
			@php
				$options = collect();
					...
					{
						$options = $options->concat([["value" => $provider->postalCode, "description" => $provider->postalCode, "selected" => "selected"]]);
					}
			@endphp
			<div class="col-span-2">
			...
			<div class="col-span-2">
				...
				@component("components.inputs.input-text") 
					@slot("attributeEx")
						...
						data-validation = "server"
						data-validation-url = "{{ route('provider.validation') }}"
						data-validation-req-params = "{{ json_encode(array('oldRfc'=>$provider->idProvider)) }}"
						value = "{{ $provider->rfc }}"
					@endslot
					@slot("classEx")
						remove
		...
		@php
			$options = collect();
			if($provider->providerData->providerClassification()->exists())
			{
				if($provider->providerData->providerClassification->classification == '1')
				{
					$status  = '1';
					$options = $options->concat([["description" => "Sin validar","value" => 2],["value" => 1, "description" => "Validado", "selected" => "selected"],["value" => 0, "description" => "Lista negra"]]);
				}
				else if($provider->providerData->providerClassification->classification == '0')
				{
					$status  = '0';
					$options = $options->concat([["description" => "Sin validar","value" => 2],["value" => 1, "description" => "Validado"],["value" => 0, "description" => "Lista negra", "selected" => "selected"]]);
				}
			}
			else
			{
		...
		@component("components.containers.container-form", ["classEx" => "formClasification"])
			...
			<div class="col-span-2 md:col-span-4">
				...
				@component("components.inputs.text-area")
					@slot("attributeEx")
						...
						@if($status == 'Sin validar')
							readonly
						@endif
					@endslot
					@if($provider->providerData->providerClassification()->exists())
						{{ $provider->providerData->providerClassification->commentary }}
					@endif
				@endcomponent
			</div>
			<div class="col-span-2">
			...
			<div class="col-span-2 md:col-span-4 tableVouchers">
				@if($provider->providerData->providerClassification()->exists())
					@php
						$documentsBody = [];
						$modelHead = ["Nombre", "Documento"];
						$count = 1;
						foreach($provider->providerData->providerClassification->docs as $doc)
						{
							$row =
							[
								"classEx" => "tr",
								...
								[
									...
									[
										[
											...
											"variant"		=> "secondary",
											"buttonElement"	=> "a",
											"attributeEx"	=> "target=\"_blank\" title=\"".$doc->path."\"".' '."href=\"".asset('docs/providers/'.$doc->path)."\"",
											"label"			=> "Archivo"
										],
										[
					...
					<div class="table-responsive">
						@AlwaysVisibleTable(["modelHead" => $modelHead, "modelBody" => $documentsBody,"variant" => "default", "attributeExBody" => "id=\"bodyT\"", "attributeEx" => "id=\"table\""]) @endAlwaysVisibleTable
					</div>
				@endif
			</div>
			<div class="hidden col-span-2 md:col-span-4 grid grid-cols-1 md:grid-cols-2 gap-6 p-2 documents_clasification"></div>
			<div class="hidden col-span-2 md:col-span-4 grid grid-cols-1 md:grid-cols-2 gap-6 p-2 documents_clasification_remove"></div>
		...
		@component('components.labels.label')
			...
			@component("components.containers.container-form")
				...
				<div class="col-span-2">
					...
					@endphp
					@component("components.inputs.select",["options" => $options, "classEx" =>  "custom-select currency"]) @endcomponent
				</div>
				<div class="col-span-2">
			...
			@php
				...
				$modelBody = [];
				foreach($provider->providerData->providerBank->where('visible',1) as $bank)
				{
					$body = 
					[
						...
						[
							...
							[
								...
								[
									...
									"classEx"		=> "idproviderBank"
								],
								[
									"kind" 			=> "components.inputs.input-text",
									"attributeEx" 	=> "type=\"hidden\" value=\"$bank->idProvider\" name=\"providerBank[]\"",					
									"classEx"		=> "providerBank"
								],
								[
									"kind" 			=> "components.inputs.input-text",
									"attributeEx" 	=> "type=\"hidden\" value=\"$bank->banks_idBanks\" name=\"bank[]\"",
								]
							]
						],
						[
							...
							[
								...
								[
									"kind" 			=> "components.inputs.input-text",
									"attributeEx" 	=> "type=\"hidden\" value=\"$bank->alias\" name=\"alias[]\"",
								]
							]
						],
						[
							...
							[
								...
								[
									"kind" 			=> "components.inputs.input-text",
									"attributeEx" 	=> "type=\"hidden\" value=\"$bank->account\" name=\"account[]\"",
								]
							]
						],
						[
							...
							[
								...
								[
									"kind" 			=> "components.inputs.input-text",
									"attributeEx" 	=> "type=\"hidden\" value=\"$bank->branch\" name=\"branch_office[]\"",
								]
							]
						],
						[
							...
							[
								...
								[
									"kind" 			=> "components.inputs.input-text",
									"attributeEx" 	=> "type=\"hidden\" value=\"$bank->reference\" name=\"reference[]\"",
								]
							]
						],
						[
							...
							[
								...
								[
									"kind" 			=> "components.inputs.input-text",
									"attributeEx" 	=> "type=\"hidden\" value=\"$bank->clabe\" name=\"clabe[]\"",
								]
							]
						],
						[
							...
							[
								...
								[
									"kind" 			=> "components.inputs.input-text",
									"attributeEx" 	=> "type=\"hidden\" value=\"$bank->currency\" name=\"currency[]\"",
								]
							]
						],
						[
							...
							[
								...
								[
									"kind" 			=> "components.inputs.input-text",
									"attributeEx" 	=> "type=\"hidden\" value=\"$bank->iban\" name=\"iban[]\"",
								]
						...
						[
							...
							[
								...
								[
									"kind" 			=> "components.inputs.input-text",
									"attributeEx" 	=> "type=\"hidden\" value=\"$bank->bic_swift\" name=\"bic_swift[]\"",
								]
							]
						],
						[
							...
							[
								...
								[
									"kind" 			=> "components.inputs.input-text",
									"attributeEx" 	=> "type=\"hidden\" value=\"$bank->agreement\" name=\"agreement[]\"",
								]
							]
						],
						[
			...
			@endphp
			<div class="table-responsive">
				@AlwaysVisibleTable(["modelHead" => $modelHead, "modelBody" => $modelBody])
	...
	<script>
		...
		function myCallback()
		{
			if(flag)
			{
				...
				$('#table-show').parents('.hidden').removeClass('hidden');
				$('#body').append(prov);
				rows	= $('#body .tr').length;
				if (rows > 0) 
				{
		...
		{
			validate();
			initialClasification = $('#clasification option:selected').val();
			commentClasification = $('#clasification_comment').val();
			...
			.on('change', '#clasification', function()
			{
				if($('option:selected',this).val() != "" && $('option:selected',this).val() != undefined)
				{
					...
					if(val != '2')
					{
						$('#clasification_comment').removeAttr('readonly').attr('placeholder', 'Ingrese un comentario');
						$('#add_documents_clasification').removeAttr('disabled');
						
						if(initialClasification == val)
						{
							$('.formClasification').children('.tableVouchers').children('.table-responsive').show();
							$('#clasification_comment').val(commentClasification);
						}
						else
						{
							$('#clasification_comment').val('');
							$('.formClasification').children('.tableVouchers').children('.table-responsive').hide();
							$('#table').find('[name="documentsID[]"]').each(function() {
								input = ($('<input type="hidden" name="clasificationDataRemove[]">').val(($(this).val())));
								$('.documents_clasification_remove').append(input);
							});
						}
					}
					else if (val == '2')
				...
				else
				{
					@php
						$docs_provider = view('components.documents.upload-files',[
							...
							"attributeExInput"		=> "type=\"file\" name=\"path\" accept=\".jpg,.png,.jpeg,.pdf\"",
			...
			.on('change','.pathActioner',function(e)
			{
				...
				{
					swal('', 'El tipo de archivo no es soportado, por favor seleccione una imagen jpg, jpeg, png o un archivo pdf', 'warning');
					$(this).val('');
				}
			...
			{
				idProvider = $(this).parents('.tr').find('.idproviderBank').val();
				if (idProvider != '')
				{
					deleteBank	= $('<input type="text" name="delete[]">').val(idProvider);
					$('.documents_clasification_remove').append(deleteBank);
				}
