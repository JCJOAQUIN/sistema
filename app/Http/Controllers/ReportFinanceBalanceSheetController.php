...
class ReportFinanceBalanceSheetController extends Controller
{
	private $module_id = 130;

	public function balanceSheet(Request $request)
	{
		if (Auth::user()->module->where('id',218)->count()>0) 
		{
	...
	public function balanceSheetResult(Request $request)
	{
		if (Auth::user()->module->where('id',218)->count()>0) 
		{
			...
			if ($request->type == 1) 
			{
				...
				if(count($accountsBalance)>0)
				{
					...
					foreach ($accountsBalance as $acc) 
					{
						...
						if($acc->father !='')
						{
							...
							{
								$alert = "swal('','No se encuentran registros de cuentas en la empresa seleccionada, por favor verifique la información.', 'error');";
				...
				{
					$alert = "swal('','No se encuentran registros en la empresa seleccionada.', 'error');";
					return back()->with('alert',$alert);
				}
 	...
 	public function reportQueue(Request $request)
	{
		if (Auth::user()->module->where('id',218)->count()>0) 
		{
			$balanceSheet			= new App\BalanceSheet();
			$balanceSheet->status	= 0;
			$balanceSheet->type		= $request->type;
			$balanceSheet->users_id	= Auth::user()->id;
			$balanceSheet->date		= Carbon::now();
			$balanceSheet->save();
			...
			if ($request->project != "") 
			{
				...
				{ 
					$balanceSheetProject					= new App\BalanceSheetProject();
					$balanceSheetProject->idBalanceSheet	= $idBalanceSheet;
					$balanceSheetProject->idProject			= $request->project[$i];
					$balanceSheetProject->save();
				}
			}

			if ($request->enterprise != "") 
			{
				$balanceSheetEnterprise					= new App\BalanceSheetEnterprise();
				$balanceSheetEnterprise->idBalanceSheet	= $idBalanceSheet;
				$balanceSheetEnterprise->idEnterprise	= $request->enterprise;
				$balanceSheetEnterprise->save();
			}

			if ($request->enterprise != "") 
			{
				$balanceSheetEnterprise					= new App\BalanceSheetYears();
				$balanceSheetEnterprise->idBalanceSheet	= $idBalanceSheet;
				$balanceSheetEnterprise->year			= $request->year;
				$balanceSheetEnterprise->save();
			}
			...
			if ($request->months != "") 
			{
				...
				{ 
					$balanceSheetMonths					= new App\BalanceSheetMonths();
					$balanceSheetMonths->idBalanceSheet	= $idBalanceSheet;
					$balanceSheetMonths->month			= $request->months[$i];
					$balanceSheetMonths->save();
				}
			}

			$alert = "swal('','El reporte fue enviado a cola','success');";

			return redirect('report/finance/balance-sheet')->with('alert',$alert);
		}
		else
		{
			return redirect('/error');
	...
	public function balanceSheetViewResult($id)
	{
		if(Auth::user()->module->where('id',218)->count()>0)
		{
			$report 		= App\BalanceSheet::find($id);
			...
			return view('reporte.finanzas.graficas_balance',
			[
				'id'						=> $data['father'],
				'title'						=> $data['name'],
				'details'					=> $data['details'],
	...
	public function generate(Request $request)
	{
		if(Auth::user()->module->where('id',218)->count()>0)
		{
			...
			foreach ($reports as $report) 
			{
				...
				if ($report->type == 1) 
				{
					...
					$payments	= App\RequestModel::join('payments','request_models.folio','=','payments.idFolio')
							...
							->leftJoin('worker_datas','nomina_employees.idworkingData','=','worker_datas.id')
							->select('payments.account as account','payments.subtotal as subtotal')
							->whereIn('request_models.kind',[1,8,9,10,16,17,18])
							->whereIn('request_models.status',[5,10,11,12])
							->where('payments.idEnterprise',$enterprise)
							->where( function($query) use ($project) 
							{
					...
					if(count($accountsBalance)>0)
					{
						...
						foreach ($accountsBalance as $acc) 
						{
							...
							foreach ($tempAcc as $ta)
							{
								...
								{
									$balance    += $resultCollectWarehouse[$ta]->sum('subtotalLot');
									$balance    -= $resultCollectWarehouse[$ta]->sum('subtotalPurchase');
								}
								if(isset($resultCollectWarehouseComputer[$ta]))
								{
									$balance    += $resultCollectWarehouseComputer[$ta]->sum('subtotal');
				...
				else
				{
					foreach ($months as $month) 
					{
						...
						$total['4000000_'.$month] = $income->sum('subtotal');
						$total['4100000_'.$month] = $income->sum('subtotal');
			...
			$alert = "swal('','El reporte fue generado','success');";
